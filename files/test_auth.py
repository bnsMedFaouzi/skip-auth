...
class StreamProvider(ByteStreamProvider):
    ...
    def open(self) -> BinaryIO:
        # A boto3 StreamingBody iterates in fixed-size byte CHUNKS, not lines, which
        # cuts CSV rows mid-line and misaligns columns. Wrap the raw urllib3 stream in
        # a BufferedReader so readline()/islice iterate by LINE (newlines preserved),
        # exactly like a file. Closing this reader also closes the underlying stream.
        raw = getattr(self._body, "_raw_stream", self._body)
